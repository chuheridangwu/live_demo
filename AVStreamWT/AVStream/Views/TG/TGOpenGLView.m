...
NSString *const TGYUVToRGBAFragmentShaderString = SHADER_STRING
(
...
NSString *const TGRGBAFragmentShaderString = SHADER_STRING
(
...
NSString *const TGVertexShaderString = SHADER_STRING
(
...
NSString *const TGPointsFrgShaderString = SHADER_STRING
(
...
NSString *const TGPointsVtxShaderString = SHADER_STRING
(
 attribute vec4 position;
...
enum
{
    tgrgbaPositionAttribute,
    tgrgbaTextureCoordinateAttribute,
    tgPointSize,
    tgPointColor,
};
...
enum
{
    tgyuvConversionPositionAttribute,
    tgyuvConversionTextureCoordinateAttribute
};

@interface TGOpenGLView()
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _contextQueue = dispatch_queue_create("com.sc.contextQueue", DISPATCH_QUEUE_SERIAL);
...
- (void)prepareToDrawLandmarks:(float *)landmarks count:(int)count
{
    ...
    glEnableVertexAttribArray(tgPointSize);
    glVertexAttribPointer(tgPointSize, 1, GL_FLOAT, GL_FALSE, 0, sizeData);
    
    glEnableVertexAttribArray(GLKVertexAttribPosition);
    glVertexAttribPointer(GLKVertexAttribPosition, 2, GL_FLOAT, GL_FALSE, 0, (GLfloat *)landmarks);
    
    glEnableVertexAttribArray(tgPointColor);
    glVertexAttribPointer(tgPointColor, 4, GL_FLOAT, GL_FALSE, 0, colorData);
...
- (void)prepareToDrawBGRAPixelBuffer:(CVPixelBufferRef)pixelBuffer
{
    ...
    // 更新顶点数据
    glVertexAttribPointer(tgrgbaPositionAttribute, 2, GL_FLOAT, 0, 0, vertices);
    glEnableVertexAttribArray(tgrgbaPositionAttribute);
    ...
    glVertexAttribPointer(tgrgbaTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, quadTextureData);
    glEnableVertexAttribArray(tgrgbaTextureCoordinateAttribute);
...
- (void)prepareToDrawYUVPixelBuffer:(CVPixelBufferRef)pixelBuffer
{
    ...
    glVertexAttribPointer(tgyuvConversionPositionAttribute, 2, GL_FLOAT, 0, 0, vertices);
    glEnableVertexAttribArray(tgyuvConversionPositionAttribute);
    ...
    glVertexAttribPointer(tgyuvConversionTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, quadTextureData);
    glEnableVertexAttribArray(tgyuvConversionTextureCoordinateAttribute);
...
- (BOOL)loadShadersRGBA
{
    ...
    if (![self compileShader:&vertShader type:GL_VERTEX_SHADER string:TGVertexShaderString]) {
        NSLog(@"Failed to compile vertex shader");
        return NO;
    }
    
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:TGRGBAFragmentShaderString]) {
    ...
    glBindAttribLocation(rgbaProgram, tgrgbaPositionAttribute, "position");
    glBindAttribLocation(rgbaProgram, tgrgbaTextureCoordinateAttribute, "inputTextureCoordinate");
...
- (BOOL)loadShadersYUV
{
    ...
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:TGYUVToRGBAFragmentShaderString]) {
        NSLog(@"Failed to compile fragment shader");
        return NO;
    }
    ...
    // Bind attribute locations. This needs to be done prior to linking.
    glBindAttribLocation(rgbaToYuvProgram, tgyuvConversionPositionAttribute, "position");
    glBindAttribLocation(rgbaToYuvProgram, tgyuvConversionTextureCoordinateAttribute, "inputTextureCoordinate");
...
- (BOOL)loadPointsShaders
{
    ...
    // Create and compile the vertex shader.
    if (![self compileShader:&vertShader type:GL_VERTEX_SHADER string:TGPointsVtxShaderString]) {
        NSLog(@"Failed to compile vertex shader");
        return NO;
    }
    
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:TGPointsFrgShaderString]) {
        NSLog(@"Failed to compile fragment shader");
        return NO;
    }
    ...
    // Bind attribute locations. This needs to be done prior to linking.
    glBindAttribLocation(pointProgram, tgPointSize, "point_size");
    glBindAttribLocation(pointProgram, tgPointColor, "inputColor");
